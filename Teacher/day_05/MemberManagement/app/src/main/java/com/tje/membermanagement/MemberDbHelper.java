...
public class MemberDbHelper extends SQLiteOpenHelper {
    ...
    public static class MemberInfo implements BaseColumns {
        ...
        public static final String COLUMN_NAME_REGIST_DATE = "registDate";
    }
    private static final String SQL_CREATE_TABLE_MEMBER =
            String.format("create table %s (%s text primary key, %s text, %s text, %s int, %s text, %s text, %s text)",
                    MemberInfo.TABLE_NAME, MemberInfo.COLUMN_NAME_ID,
                    MemberInfo.COLUMN_NAME_PASSWORD, MemberInfo.COLUMN_NAME_NAME,
                    MemberInfo.COLUMN_NAME_AGE, MemberInfo.COLUMN_NAME_PHONE,
                    MemberInfo.COLUMN_NAME_ADDRESS, MemberInfo.COLUMN_NAME_REGIST_DATE);
    ...
    public ArrayList<Member> select() {
        ...
        String [] columns = {
                ...
                MemberInfo.COLUMN_NAME_REGIST_DATE
        };

        Cursor cursor = db.query(MemberInfo.TABLE_NAME,
                columns, null, null,
                null, null, null, null);
        ...
        while( cursor.moveToNext() ) {
            ...
            member.setId(
                    cursor.getString(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_ID)));
            member.setPassword(
                    cursor.getString(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_PASSWORD)));
            member.setName(
                    cursor.getString(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_NAME)));
            member.setAge(
                    cursor.getInt(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_AGE)));
            member.setPhone(
                    cursor.getString(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_PHONE)));
            member.setAddress(
                    cursor.getString(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_ADDRESS)));
            member.setRegistDate(
                    cursor.getString(
                            cursor.getColumnIndex(MemberInfo.COLUMN_NAME_REGIST_DATE)));
            result.add(member);
        }
